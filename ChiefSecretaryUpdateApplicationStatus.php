...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Chief_Head_Aprv_RM = ?, Chief_Head_time_stamp = NOW(), Chief_Head_user_id = ? WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    if ($offi_cat == 1) {
        $app_status = 3; // Approved status for offi_cat = 1
    } elseif ($offi_cat == 2) {
        $app_status = 111; // Approved status for offi_cat = 2
    }elseif ($offi_cat == 3) {
        $app_status = 111; // Approved status for offi_cat = 3
    } elseif ($offi_cat == 4) {
        $app_status = 111; // Approved status for offi_cat = 4
    }elseif ($offi_cat == 5) {
        $app_status = 111; // Approved status for offi_cat = 5
    }elseif ($offi_cat == 6) {
        $app_status = 111; // Approved status for offi_cat = 6
    }else {
        echo json_encode(['success' => false, 'error' => 'Invalid offi_cat value']);
        exit();
    }
    // Update application status and rejection reason
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Chief_Head_Reject_RM = ?, Chief_Head_time_stamp = NOW(), Chief_Head_user_id = ? WHERE app_no = ?");
