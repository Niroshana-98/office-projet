...
// Fetch the min_id from the office table
...
// Step 2: Fetch applications where c_w_p matches admin min_id and app_status = 110
$applicationsQuery = "
    SELECT 
        application.app_no, 
    ...
    WHERE 
        application.min_id = ? 
        AND application.app_status IN (110, 111)
";
$stmt = $conn->prepare($applicationsQuery);

if ($stmt) {
    $stmt->bind_param("i", $minId); // Bind admin min_id parameter
    $stmt->execute();
