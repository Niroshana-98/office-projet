...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, ACC2_Aprv_RM = ?, ACC2_time_stamp = NOW(), ACC2_user_id = ? WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, ACC2_Reject_RM = ?, ACC2_time_stamp = NOW(), ACC2_user_id = ? WHERE app_no = ?");
