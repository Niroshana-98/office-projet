...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, CS_Chk_Offi_Aprv_RM = ?, CS_Chk_Offi_time_stamp = NOW(), CS_Chk_Offi_user_id = ?, AO_Reject_RM = NULL WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, CS_Chk_Offi_Reject_RM = ?, CS_Chk_Offi_time_stamp = NOW(), CS_Chk_Offi_user_id = ?, AO_Reject_RM = NULL WHERE app_no = ?");
