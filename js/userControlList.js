document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('userControlList_to_db.php')
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            ...
            if (tableBody) {
                tableBody.innerHTML = `
                    <tr>
                        <td colspan="4" class="text-center text-danger">Failed to load Users List. Please try again later.</td>
                    </tr>
                `;
            }
        });
    ...
    function renderTable(data) {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="4" class="text-center">No new Users found.</td>
                </tr>
            `;
        }
    }
...
function viewApplication(user_id) {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = `userControlListForm.php?user_id=${user_id}`;
        } else {
