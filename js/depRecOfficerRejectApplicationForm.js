document.addEventListener("DOMContentLoaded", function () {
    fetch("depRecOfficerRejectApplicationForm_to_db.php", {
        credentials: 'include'
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            const rejectDepRecOffiDiv = document.getElementById("rejectDepRecOffiDiv");

            if(!data.Dep_Rec_Offi_Reject_RM) {
                rejectDepRecOffiDiv.style.display = "none";
            } else {
                document.getElementById("rejectDepRecOffi").value = data.Dep_Rec_Offi_Reject_RM;
