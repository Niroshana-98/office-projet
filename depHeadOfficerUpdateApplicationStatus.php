...
$recommendation = isset($data['Dep_head_Recommend']) ? $data['Dep_head_Recommend'] : '';
...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Dep_head_Recommend = ?, Dep_head_Aprv_RM = ?, Dep_head_time_stamp = NOW(), Dep_head_user_id = ?, Min_Chk_Offi_Reject_RM = NULL WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Dep_head_Reject_RM = ?, Dep_head_time_stamp = NOW(), Dep_head_user_id = ?, Min_Chk_Offi_Reject_RM = NULL WHERE app_no = ?");
