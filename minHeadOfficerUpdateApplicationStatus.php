...
$recommendation = isset($data['Min_head_Recommend']) ? $data['Min_head_Recommend'] : '';
...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Min_head_Recommend = ?, Min_head_Aprv_RM = ?, Min_head_time_stamp = NOW(), Min_head_user_id = ?, CS_Chk_Offi_Reject_RM = NULL WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Min_head_Reject_RM = ?, Min_head_time_stamp = NOW(), Min_head_user_id = ?, CS_Chk_Offi_Reject_RM = NULL WHERE app_no = ?");
