...
if ($status == 1) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Dep_Rec_Offi_Aprv_RM = ?, Dep_Rec_Offi_time_stamp = NOW(), Dep_Rec_Offi_user_id = ?, Dep_head_Reject_RM = NULL WHERE app_no = ?");
...
if ($status == 2 && !empty($comment)) {
    ...
    $stmt = $conn->prepare("UPDATE application SET app_status = ?, Dep_Rec_Offi_Reject_RM = ?, Dep_Rec_Offi_time_stamp = NOW(), Dep_Rec_Offi_user_id = ? WHERE app_no = ?");
